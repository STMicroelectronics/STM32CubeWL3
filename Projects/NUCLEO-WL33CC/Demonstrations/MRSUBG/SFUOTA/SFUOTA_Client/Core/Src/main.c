...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
static void OTA_Try_Run_Downloaded_Application(void)
{
  uint32_t applicationAddress;
  OTA_Check_ServiceManager_Operation();

  applicationAddress = OTA_Check_Application_Tags_Value();
  if (applicationAddress == APP_WITH_OTA_SERVICE_ADDRESS)
  {
...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
  OTA_Try_Run_Downloaded_Application();
