...
/* USER CODE BEGIN PV */
static __attribute__((aligned(4))) uint8_t RxPacketBuffer[MAX_PACKET_LENGTH];
static uint16_t RxReceivedByteCount;
static volatile uint8_t RxOkEventPending;
static volatile uint8_t RxErrorEventPending;
static volatile uint32_t RxErrorIrqStatus;
/* USER CODE END PV */
...
uint32_t MX_APPE_Init(void *p_param)
{
  ...
  __HAL_MRSUBG_SET_DATABUFFER0_POINTER((uint32_t)RxPacketBuffer);
  ...
  printf("STM32WL3 SFUOTA Rx Demo.\r\n");
...
void HAL_MRSubG_IRQ_Callback(void)
{
  ...
  if (irq_status & RFSEQ_IRQ_ERROR_MASK)
  {
    RxErrorIrqStatus = (irq_status & RFSEQ_IRQ_ERROR_MASK);
    __HAL_MRSUBG_CLEAR_RFSEQ_IRQ_FLAG(RFSEQ_IRQ_ERROR_MASK);
...
void MX_APPE_Process(void)
{
  ...
  if (RxOkEventPending == TRUE)
  {
    ...
    for (volatile uint16_t i = 0U; i < RxReceivedByteCount; i++)
    {
      printf("%02X ", RxPacketBuffer[i]);
    }
    printf("\r\n");
    __HAL_MRSUBG_STROBE_CMD(CMD_RX);
  }
  ...
  if (RxErrorEventPending == TRUE)
  {
    RxErrorEventPending = FALSE;
    BSP_LED_Toggle(LD2);
    printf("RX error IRQ status: 0x%08lX\r\n", RxErrorIrqStatus);
    RxErrorIrqStatus = 0U;
    __HAL_MRSUBG_STROBE_CMD(CMD_RX);
  }
