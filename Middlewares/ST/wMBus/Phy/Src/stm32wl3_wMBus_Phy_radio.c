...
/**
  ...
  */
const uint32_t WmBus_responseDelay[WMBUS_MODE_NUMBER][2] =
{
...
/**
  ...
  */
const uint32_t wMBus_FACTransmissionDelay[WMBUS_MODE_NUMBER] = 
{
...
SMRSubGConfig wMBus_Phy_MRSUBG_RadioInitStruct = {0};

MRSubG_PcktBasicFields wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct = {0};
...
static wMBus_Phy_debug_t MRSubG_WmBus_debug_log[WMBUS_LOG_BUFFER_SIZE]; /**< Debug log buffer */
#endif

wMBus_Phy_tRO_t WmBus_tRO = {0}; /**< tRO structure */
uint32_t wMBus_Phy_tTxD = 0;     /**< tTxD: FAC Transmission delay */
...
void wMBus_Phy_init(uint8_t WmBus_mode, uint8_t WmBus_Direction, uint8_t WmBus_Format)
{
  ...
  wMBus_Phy_MRSUBG_RadioInitStruct.lFrequencyBase = WmBus_Frequency[WmBus_mode][WmBus_Direction];
  wMBus_Phy_MRSUBG_RadioInitStruct.xModulationSelect = WmBus_Modulation[WmBus_mode][WmBus_Direction];
  wMBus_Phy_MRSUBG_RadioInitStruct.lDatarate = WmBus_Datarate[WmBus_mode][WmBus_Direction];
  wMBus_Phy_MRSUBG_RadioInitStruct.lFreqDev = WmBus_FreqDev[WmBus_mode][WmBus_Direction];
  wMBus_Phy_MRSUBG_RadioInitStruct.lBandwidth = WmBus_ChannelBandwidth[WmBus_mode][WmBus_Direction];
  wMBus_Phy_MRSUBG_RadioInitStruct.dsssExp = 0;
  wMBus_Phy_MRSUBG_RadioInitStruct.PADrvMode = PA_DRV_TX_HP;
  /* +10dBm Tx output power */
#ifdef IS_169MHZ
  wMBus_Phy_MRSUBG_RadioInitStruct.outputPower = -8;
#else
  wMBus_Phy_MRSUBG_RadioInitStruct.outputPower = 10;
#endif

  wMBus_Phy_MRSubG_MspInit();
  HAL_MRSubG_Init(&wMBus_Phy_MRSUBG_RadioInitStruct);

  /* Configure Timing parameters */
  WmBus_tRO.tRO_min = WmBus_responseDelay[WmBus_mode][0];
  WmBus_tRO.tRO_max = WmBus_responseDelay[WmBus_mode][1];
  wMBus_Phy_tTxD = wMBus_FACTransmissionDelay[WmBus_mode];

  /* Configures the packet parameters */
  wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.PreambleLength = WmBus_PreambleLength[WmBus_mode][WmBus_Direction];
  wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.PostambleLength = PREAMBLE_BYTE(0);
  /* set Tx and Rx Sync settings - by default same but can be different in some special cases */
  wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.SyncLength = WmBus_SyncLength[WmBus_mode][WmBus_Direction] - 1;
  wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.SyncWord = WmBus_SyncPattern[WmBus_mode][WmBus_Direction];
  ...
  {
    wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.SyncWord = WMBUS_SYNC_PATTERN_C_MODE_M2O_FORMAT_B;
  }

  wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.FixVarLength = FIXED;
  wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.PreambleSequence = PRE_SEQ_0101;
  wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.PostambleSequence = POST_SEQ_0101;
  wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.CrcMode = PKT_NO_CRC;
  wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.Coding = (MRSubG_PcktCoding)WmBus_ChannelCoding[WmBus_mode][WmBus_Direction];
  /* set Manchester type1 if needed */
  if (wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.Coding == CODING_MANCHESTER)
  {
    LL_MRSubG_PacketHandlerManchesterType(MANCHESTER_TYPE1);
  }
  wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.DataWhitening = DISABLE;
  wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.LengthWidth = BYTE_LEN_1;
  wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.SyncPresent = ENABLE;
  ...
  if ((WmBus_mode == C_MODE) || (WmBus_mode == T_MODE))
  {
    ...
    wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.SyncWord = WMBUS_SYNC_PATTERN_T1C1_MODE_M2O_GATEWAY;
    wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.SyncLength = WMBUS_SYNC_LENGTH_T1C1_MODE_M2O_GATEWAY - 1;

    /* force NRZ decoding */
    wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct.Coding = CODING_NONE;
  }
#endif
  ...
  HAL_MRSubG_PacketBasicInit(&wMBus_Phy_MRSUBG_MRSUBG_PacketSettingsStruct);
  ...
  HAL_MRSubG_SetPALeveldBm(7, wMBus_Phy_MRSUBG_RadioInitStruct.outputPower, wMBus_Phy_MRSUBG_RadioInitStruct.PADrvMode);

  /* check back */
  wMBus_Phy_MRSUBG_RadioInitStruct.outputPower = HAL_MRSubG_GetPALeveldBm();
...
void wMBus_Phy_IRQHandler(void)
{
  ...
  if (irq & MR_SUBG_GLOB_STATUS_RFSEQ_IRQ_STATUS_RX_ALMOST_FULL_0_F)
  {
    ...
    if (xRxState == SM_RX_NEW_PACKET)
    {
...
#ifdef WMBUS_DEBUG
        wMBus_Phy_trace_debug_log(DEBUG_EVENT_WMBUS_RX_VALID_HEADER_ERROR_L_FIELD, cLField, Hal_MrSubGhz_Frame_format, 0);
#endif
      ...
      else
      {
        ...
        if (!wMBus_Phy_first_CRC_check())
        {
          ...
          __HAL_MRSUBG_CLEAR_RFSEQ_IRQ_FLAG(MR_SUBG_GLOB_STATUS_RFSEQ_IRQ_STATUS_RX_ALMOST_FULL_0_F);
#ifdef WMBUS_DEBUG
          wMBus_Phy_trace_debug_log(DEBUG_EVENT_WMBUS_FIRST_CRC_ERROR, cLField, Hal_MrSubGhz_Frame_format, 0);
#endif
          __HAL_MRSUBG_STROBE_CMD(CMD_SABORT);
#ifdef WMBUS_DEBUG
          wMBus_Phy_trace_debug_log(DEBUG_COMMAND_SABORT_LOG, 0, 0, 0);
    ...
    else if (xRxState == SM_RX_LAST)
    {
      ...
      *(HAL_buffer_size_ptr) = wMBus_Phy_wait_Rx_completed(HAL_buffer_ptr, HAL_RssiDbm_ptr, HAL_WmBus_RxSync_ptr);
#else
      *(HAL_buffer_size_ptr) = wMBus_Phy_wait_Rx_CRC_mngt(HAL_buffer_ptr, HAL_RssiDbm_ptr, *(HAL_buffer_format_ptr), HAL_WmBus_RxSync_ptr);
#endif
...
/**
  ...
  * @param  LL_WmBus_RxSync_ptr Pointer to the wM-Bus RX sync value.
